...
class NeuronPool_Separable_Pool3d_200_small(nn.Module):
    ...
    def __init__(self, multiply_factor: float = 10.):
        ...
        self.bottom = nn.Sequential(
                nn.Conv3d(in_channels = 2, out_channels = 4, kernel_size = (5, 7, 7), stride = 1, padding = (0, 3, 3), bias = False, padding_mode = 'replicate'),
                MultiplyBy(multiply_factor),
                neuron.IFNode(),
                )

        self.conv1 = PoolingEncoder(in_channels = 4, out_channels = 8, kernel_size = 7, multiply_factor = multiply_factor)
        self.conv2 = PoolingEncoder(in_channels = 8, out_channels = 16, kernel_size = 7, multiply_factor = multiply_factor)
        self.conv3 = PoolingEncoder(in_channels = 16, out_channels = 32, kernel_size = 7, multiply_factor = multiply_factor)
        self.conv4 = PoolingEncoder(in_channels = 32, out_channels = 64, kernel_size = 7, multiply_factor = multiply_factor)


        # Residual Block

        self.res1 = SeparableSEWResBlock(in_channels = 64, multiply_factor = multiply_factor, kernel_size = 7)


        # Decoder Block

        self.deconv4 = separable_decoder_block(in_channels = 64, out_channels = 32, up_size = (25, 25),  kernel_size = 7, multiply_factor = multiply_factor)
        self.deconv3 = separable_decoder_block(in_channels = 32 * 2, out_channels = 16, up_size = (50, 50),  kernel_size = 7, multiply_factor = multiply_factor)
        self.deconv2 = separable_decoder_block(in_channels = 16 * 2, out_channels = 8, up_size = (100, 100),  kernel_size = 7, multiply_factor = multiply_factor)
        self.deconv1 = separable_decoder_block(in_channels = 8 * 2, out_channels = 4, up_size = (200, 200),  kernel_size = 7, multiply_factor = multiply_factor)

        # Predictor Block


        self.pred4 = separable_predictor_block(in_channels = 32 * 2, out_channels = 2, up_size = (200, 200), kernel_size = 7)
        self.pred3 = separable_predictor_block(in_channels = 16 * 2, out_channels = 2, up_size = (200, 200), kernel_size = 7)
        self.pred2 = separable_predictor_block(in_channels = 8 * 2, out_channels = 2, up_size = (200, 200), kernel_size = 7)
        self.pred1 = separable_predictor_block(in_channels = 4 * 2, out_channels = 2, up_size = (200, 200), kernel_size = 7)
...
if __name__ == '__main__':
    ...
    in_tensor = torch.randn(1, 2, 21, 200, 200)
